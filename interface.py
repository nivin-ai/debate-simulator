...
if st.button("Start Debate"):
    ...
    if pro_argument_safety == None:
        ...
        with col2:
            ...
            for claim in pro_claim:
                st.markdown(f"<div class='feedback'>Claim: {pro_claim}<br>Accuracy: {fact_checker.rate_claim(pro_claim)}<br><br></div>",unsafe_allow_html=True)
    else:
    ...
    if con_argument_safety == None:
        ...
        with col2:
            ...
            st.markdown(f"<div class='feedback'>Claim: {con_claim}<br>Accuracy: {fact_checker.rate_claim(con_claim)}</div>",unsafe_allow_html=True)
    else:
